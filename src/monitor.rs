...
mod macos {
    ...
    use std::{ptr, thread, ffi::c_void, sync::Arc, sync::atomic::{AtomicU32, Ordering}};
    ...
    extern "C" {
        fn IORegisterForSystemPower(r: *mut c_void, p: *mut *mut c_void, c: Option<unsafe extern "C" fn(*mut c_void, u32, u32, *mut c_void)>, n: *mut u32) -> u32;
    ...
    struct State { tx: mpsc::Sender<HardwareEvent>, root_port: AtomicU32 }
    ...
    unsafe extern "C" fn power_callback(refcon: *mut c_void, _: u32, msg: u32, arg: *mut c_void) {
        ...
        match msg {
            ...
            0xE0000270 | 0xE0000280 => { // Sleep related
                let port = s.root_port.load(Ordering::SeqCst);
                if port != 0 { IOAllowPowerChange(port, arg as i64); }
            }
            _ => {}
        }
    }
    ...
    pub fn start_monitor(tx: mpsc::Sender<HardwareEvent>) {
        ...
        let _ = netwatcher::watch_interfaces(move |upd| {
            ...
            for idx in &upd.diff.added {
                if let Some(iface) = upd.interfaces.get(idx) {
                    if iface.name.starts_with("en") { info!("[Monitor] Interface added: {}", iface.name); changed = true; }
                }
            }
            for (idx, diff) in &upd.diff.modified {
                if let Some(iface) = upd.interfaces.get(idx) {
                    if iface.name.starts_with("en") && (!diff.addrs_added.is_empty() || !diff.addrs_removed.is_empty()) {
                        info!("[Monitor] Interface modified: {}", iface.name);
                        changed = true;
                    }
                }
            }
            if changed { let _ = tx_net.try_send(HardwareEvent::Refresh); }
        ...
        thread::spawn(move || unsafe {
            ...
            let s = Arc::new(State { tx, root_port: AtomicU32::new(0) });
            let refcon = Arc::into_raw(s.clone()) as *mut c_void;
            let root = IORegisterForSystemPower(refcon, &mut port, Some(power_callback), &mut notifier);
            if root != 0 {
